...
class BinaryEncoder {
    ...
    static writeSplitVarint64(buffer, lowBits, highBits) {
        ...
        assert(lowBits >= 0 && lowBits < constants_1.BinaryConstants.TWO_TO_32);
        assert(highBits >= 0 && highBits < constants_1.BinaryConstants.TWO_TO_32);
    ...
    static writeUnsignedVarint32(buffer, value) {
        ...
        assert(value >= 0 && value < constants_1.BinaryConstants.TWO_TO_32);
        while (value > 127) {
    ...
    static writeSignedVarint32(buffer, value) {
        assert(value === Math.floor(value));
        assert(value >= -constants_1.BinaryConstants.TWO_TO_31 && value < constants_1.BinaryConstants.TWO_TO_31);
        if (value >= 0) {
    ...
    static writeSignedVarint64(buffer, value) {
        assert(value === Math.floor(value));
        assert(value >= -constants_1.BinaryConstants.TWO_TO_63 && value < constants_1.BinaryConstants.TWO_TO_63);
    ...
    static writeDouble(buffer, value) {
        assert(value === Infinity
            || value === -Infinity
            || isNaN(value)
            || (value >= -constants_1.BinaryConstants.FLOAT64_MAX && value <= constants_1.BinaryConstants.FLOAT64_MAX));
    ...
    static splitInt64(value) {
        ...
        let lowBits = value >>> 0;
        let highBits = Math.floor((value - lowBits) / constants_1.BinaryConstants.TWO_TO_32);
        highBits = highBits >>> 0;
        if (sign) {
            highBits = ~highBits >>> 0;
            lowBits = ~lowBits >>> 0;
