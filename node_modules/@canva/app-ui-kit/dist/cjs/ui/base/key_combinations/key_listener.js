"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.addListenerOnBody = exports.KeyListener = void 0;
const preconditions_1 = require('../../../base/preconditions');
const input_1 = require('../dom/input/input');
const key_map_1 = require("./key_map");
const eventKinds = ['keydown', 'keyup', 'keypress'];
class KeyListener {
    constructor(element, opt) {
        this.element = element;
        this.opt = opt;
        this.events = {
            ['keydown']: [],
            ['keyup']: [],
            ['keypress']: [],
        };
    }
    static getEnglishKey(event) {
        const eventKey = event.key != null && key_map_1.KeyMap.englishKeys.has(event.key)
            ? event.key.toLowerCase()
            : key_map_1.KeyMap.codeToKey[event.code];
        if (eventKey == null) {
            return event.key;
        }
        if (event.shiftKey && !event.code.startsWith('Numpad') && eventKey >= 'a' && eventKey <= 'z') {
            return key_map_1.KeyMap.shiftMap[eventKey] ? key_map_1.KeyMap.shiftMap[eventKey] : eventKey;
        }
        return eventKey;
    }
    static isKeyMatch(event, trigger) {
        const eventKey = trigger.ignoreKeyboardLayout === false ? event.key : KeyListener.getEnglishKey(event);
        const isModifiersOtherThanShiftMatch = (event.altKey === !!trigger.altKey || eventKey === 'Alt')
            && (event.ctrlKey === !!trigger.ctrlKey || eventKey === 'Control')
            && (event.metaKey === !!trigger.metaKey || eventKey === 'Meta');
        if (!isModifiersOtherThanShiftMatch) {
            return false;
        }
        const specialChars = new Set(['*', '+']);
        if (eventKey === trigger.key) {
            if (key_map_1.KeyMap.reverseShiftMap[trigger.key] != null) {
                return specialChars.has(trigger.key) ? event.shiftKey === !!trigger.shiftKey : true;
            }
            return event.shiftKey === !!trigger.shiftKey || eventKey === 'Shift';
        }
        else if (key_map_1.KeyMap.shiftMap[trigger.key] != null && trigger.shiftKey) {
            return specialChars.has(key_map_1.KeyMap.shiftMap[trigger.key])
                ? false
                : eventKey === key_map_1.KeyMap.shiftMap[trigger.key];
        }
        return false;
    }
    addListener(kind, trigger, callback) {
        const triggerClone = { ...trigger };
        const handler = ev => {
            if (KeyListener.isKeyMatch(ev, triggerClone) && !this.shouldIgnoreEvent(ev)) {
                callback(ev);
            }
        };
        this.element.addEventListener(kind, handler);
        this.events[kind].push(handler);
    }
    shouldIgnoreEvent(ev) {
        const eventTarget = preconditions_1.Preconditions.checkExists(ev.target, 'event target must exist');
        const ownerDocument = preconditions_1.Preconditions.checkExists(eventTarget.ownerDocument, 'event target must be node and ownerDocument must exist');
        if (ev.keyCode === 229) {
            return true;
        }
        return this.opt.ignoreInput && (0, input_1.isElementInput)(eventTarget, ownerDocument.defaultView);
    }
    reset() {
        for (const eventType of eventKinds) {
            this.events[eventType].forEach(handler => this.element.removeEventListener(eventType, handler));
            this.events[eventType] = [];
        }
    }
}
exports.KeyListener = KeyListener;
let keyListenerOnBody;
function addListenerOnBody(kind, trigger, callback) {
    if (keyListenerOnBody == null) {
        keyListenerOnBody = new KeyListener(document.body, { ignoreInput: true });
    }
    keyListenerOnBody.addListener(kind, trigger, callback);
}
exports.addListenerOnBody = addListenerOnBody;
