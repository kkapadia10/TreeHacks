import { jsx as _jsx } from "react/jsx-runtime";
import { Swatch as EaselSwatch } from '../../../../../base/swatch/swatch';
/** 
 * The size of a swatch.
 */
export const swatchSizes = ['xsmall', 'small'];
/** 
 * The appearance of a swatch.
 * If a swatch uses the `"outline"` variant, it's rendered as a white box and outlined with the value of the `fill` property.
 */
export const variants = ['solid', 'outline'];
/** 
 * A selectable square that displays one or more colors.
 */
export function Swatch(props) {
    return (_jsx(EaselSwatch, { fill: props.fill, tooltipLabel: props.tooltipLabel, variant: props.variant || 'solid', onClick: props.onClick, active: props.active, size: props.size || 'small', disabled: props.disabled }));
}
