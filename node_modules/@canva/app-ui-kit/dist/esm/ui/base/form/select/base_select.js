...
import { WithKeyboardNavigation } from '../../a11y/keyboard_navigation/use_keyboard_navigation';
...
export const BaseSelect = observer(function BaseSelect(props) {
    var _a, _b;
    const [store] = React.useState(() => { var _a; return (_a = props.store) !== null && _a !== void 0 ? _a : new BaseSelectStore(); });
    const [presenter] = React.useState(() => { var _a; return (_a = props.presenter) !== null && _a !== void 0 ? _a : new BaseSelectPresenter(); });
    const { query } = store;
    const { Menu = BaseSelectMenu, open, triggerContainerRef, menuContainerRef, searchable, isSelected = defaultIsSelected, onChange, onOpen, onClose, closeOnSelection = true, } = props;
    ...
    const filterFn = (typeof searchable === 'object' && searchable.filterFn) || defaultFilterFn;
    ...
    const MenuWithSearchInput = React.useMemo(() => observer((props) => {
        return (_jsx(Menu, { ...props, header: _jsx(BaseInput, { type: "search", value: store.query, onChange: onSearchInputChange, placeholder: searchInputPlaceholder.current, icon: { type: 'search', align: 'start' }, ariaActiveDescendant: deriveIdentifier(props.id, props.activeIndex) }) }));
    ...
    React.useEffect(() => {
        return reaction(() => store.open, open => {
            ...
            if (open) {
                ...
                else if (accessModeState.isKeyboardMode) {
                    (_b = store.keyboardNavigation) === null || _b === void 0 ? void 0 : _b.moveTo(0);
                }
            }
            else {
    ...
    return (_jsx(
        ...
        { map: triggerKeyMap, handleInputs: true, children: ({ ref: triggerKeysRef }) => (_jsx(WithKeyboardNavigation, { size: flatOptions.length, handleInputs: true, onSelect: keySelectionHandler, onExit: exitHandler, onNext: setKeyboardMode, onPrev: setKeyboardMode, onFirst: setKeyboardMode, onLast: setKeyboardMode, children: keyboardNavigation => {
                    store.keyboardNavigation = keyboardNavigation;
                    return (_jsx(Observer, { children: () => (_jsx(StatelessBaseSelect, { ...props, value: value, open: open !== null && open !== void 0 ? open : store.open, activeIndex: keyboardNavigation.index > -1 ? keyboardNavigation.index : undefined, onRequestToggle: toggleMenu, onItemClick: onItemClick, onItemHover: onItemHover, focusOnMenu: !!searchable, triggerContainerRef: composeRefs(searchable ? null : keyboardNavigation.ref, triggerKeysRef, triggerContainerRef), menuContainerRef: composeRefs(searchable ? keyboardNavigation.ref : null, menuContainerRef), Menu: searchable ? MenuWithSearchInput : Menu, options: options, tooltipLabel: props.tooltipLabel, tooltipPlacement: props.tooltipPlacement })) }));
                } })) }
    ));
});
